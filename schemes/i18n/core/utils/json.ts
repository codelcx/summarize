...
{
  return Array.from({ length: count + 1 }).join(indentType)
}
...
function formatJSON(json: string, indentType: string): string
{
  ...
  // 提取反斜杠和字符串
  json = json
    .replaceAll(/\\./g, pushPlaceholder) // 处理反斜杠
    .replaceAll(/(".*?"|'.*?')/g, pushPlaceholder) // 处理字符串
    .replaceAll(/\s+/g, '') // 去除多余空格
  ...
  output = output
    .replaceAll(/\[[\d,\s]+\]/g, match => match.replaceAll(/\s/g, ''))
    .replaceAll(/\\(\d+)\\/g, popPlaceholder) // 还原填充数据
    .replaceAll(/\\(\d+)\\/g, popPlaceholder) // 再次还原填充数据
 ...
 */
export function jsonFormatter(json: object | string, config: JSONFormatConfig = configDefault): string
{
  // 确保输入是 JSON 字符串
